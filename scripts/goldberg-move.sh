...
#
# Forty-two territories - `GP(2,0)`, the third-smallest Goldberg solid - and four coloured
# disks. Click a disk to pick it up, and the same disk again to put it down. Click a territory
# and the move is laid out to it and the planet turns to face it; click that territory again to
# finish the move, or a different one to go further. There is no limit on how far.
#
...
# Release always, for the reason `goldberg-view.sh` gives: a debug build tessellates forty-two
# regions slowly enough to read as a stutter that is not there.
exec cargo run --release --manifest-path "$root/Cargo.toml" -p goldberg-move -- "$@"
