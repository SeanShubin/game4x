...
**He wants the root uncluttered and one place to browse from**, and he wants the turn-by-turn to show
what changed and what caused it rather than eight full states. **Three parts; the third is the one
that changes what he can check.**

**1. Two directories.**

- **`scenario/commands/`** and **`scenario/expected/`** - input and what it should produce, one
  directory apart. `commands/` and `expected/` move wholesale
- **`reports/`** - every generated view, markdown and HTML together, plus **`reports/index.html`**
- **`pending.md` stays in the root.** It is the lanes' index rather than one of his reports, and
  moving it means changing `hooks/pre-commit` for no gain

**2. Both formats, and the reason is `P-225` rather than completeness.** *When I change my mind I
delete the expected data and run the scenario again... what I review is the diff in version
control.* **HTML diffs badly and markdown diffs readably**, so markdown is the surface a change is
reviewed on and HTML is the surface things are browsed on. Generate both from one model. Neither is
canonical.
...
Per turn, in this order: **the commands that ran, then what changed since the previous turn, then the
full state**. The full state stays; it stops being the first thing. **Checking a transformation
means checking that these commands produced this change**, and a full dump makes him find the
difference himself, eight times.

**Assert the counts**, as always: every command in `scenario/commands/play.4x` appears in exactly one
...
vets it is one less thing moving under him; if it lands after, nothing is lost** - the content does
