...
**[`decide/proposals.md`](../../decide/proposals.md)**, with nothing else in the file. **This one has
no open section at all**, by construction: it keeps the notices addressed to other perspectives,
every item that has closed, and the ledger. A proposal arrives here only when it lands.
...
```
root.join("docs/notes/proposals.md"),     also read decide/proposals.md
root.join("docs/notes/decisions.md"),     also read decide/questions.md
```

**`tools/outbox` is production support and yours** - `CLAUDE.md`, Perspectives - so this is filed
rather than wired. `tools/spec` is this lane's and already follows.

## What moved and what did not

**`decide/` holds only what waits on a person.** `decide/proposals.md` is the open queue - three
items - and `decide/questions.md` the open questions, currently none. **127 lines of a 5,697-line
file.**

**Nothing else moved.** `docs/notes/proposals.md` keeps the ledger, every closed item, and every
notice addressed to you or to a lens - **including this one.** It is still this lane's outbox and
still where you read what it has filed. `decide/` is **not** an outbox: no instance files there.

## The consequence you will see first

**`pending.md` will stop listing the three open proposals** until those two paths are added, because
it is generated from what `tools/outbox` reads. Its *What must be decided* section is the one that
goes quiet, and it is the section Sean reads.

## One thing worth checking rather than assuming

**`shown.ends_with("proposals.md")` at `lib.rs:210`** decides something about the queue, and both
files now end that way. This lane did not read what it gates, so this names it rather than telling
you what it does.

## What this lane changed, so you are not surprised by a diff

`tools/spec`: `QUEUE`, `QUESTIONS` and `OUTBOX` as constants; `land` now removes the block from the
queue and writes the ledger row to the record, which are two files rather than one. `queue.rs` gains
`RECORD_SECTIONS` and `QUEUE_SECTIONS` beside `FILE_SECTIONS`, because the two files have different
sections and a block ends at whichever its own file has. Its tests split the same way. Green.
...
| P-470, a kind's line names its traits, derived ones included                                                                 | `spec/data/kinds.4x`, `spec/data/traits.4x`, and `spec/console.md` -> The language                                                                                                                       | 2026-09-12 |
| P-466, three columns of *Units and structures* come out, and `binding` becomes derived                                       | `releases/first-release.md` -> Units and structures, and Traits                                                                                                                                          | 2026-09-12 |
| P-472, the debug view is a fifth surface                                                                                     | `spec/interface.md` -> Surfaces                                                                                                                                                                          | 2026-09-12 |
...
| P-505, Nothing spills, because nothing was inside anything                                                                   | `spec/logistics.md` -> Containment                                                                                                                                                                       | 2026-09-14 |
| P-504, Waste is free capacity gone negative, and one sentence allows it                                                      | `spec/console.md` -> The language                                                                                                                                                                        | 2026-09-14 |
...
| P-545, `spec/` covers the shape of the artifact, not only the rules of the game                                              | `spec/README.md` -> the opening line, and Rules for this directory                                                                                                                                       | 2026-09-24 |
| P-546, The architecture check lives in `tools/spec/`, where the lane it constrains cannot edit it                            | `spec/README.md` -> Rules for this directory                                                                                                                                                             | 2026-09-24 |
| P-547, The engine adapter is a layer, and rule 4 has to say so too                                                           | `docs/architecture.md` -> The layers, and Rules                                                                                                                                                          | 2026-09-24 |
| P-548, One sentence refused during promotion, because it cites a rule that is below it                                       | `CLAUDE.md` -> Perspectives                                                                                                                                                                              | 2026-09-24 |
| P-549, An ark is never on the surface, which makes one row of the data wrong                                                 | `spec/orbit.md` -> The layers, `spec/data/limit.4x`, and `releases/first-release.md` -> What bounds a kind                                                                                               | 2026-09-24 |
| P-550, A check that pins the present state cannot report a gap against what should be                                        | `CLAUDE.md` -> What done means                                                                                                                                                                           | 2026-09-24 |
| P-553, The deploy comes first, and the launch is the winning act                                                             | `spec/control.md` -> Winning                                                                                                                                                                             | 2026-09-25 |
| P-552, An Ark holds one energy, mines one a turn, and spends one to move                                                     | `spec/units.md`, and `releases/first-release.md` -> Units and structures, Recipes                                                                                                                        | 2026-09-25 |
| P-555, A log directory, one command log, and a screen the assistant can read                                                 | `spec/interface.md` -> a new section, Logs                                                                                                                                                               | 2026-09-25 |
| P-556, `spec/control.md` becomes a future plan whole, because nothing is left in it                                          | `spec/control.md` -> `spec/future/control.md`, `spec/README.md`, and `releases/first-release.md` -> `R-6`                                                                                                | 2026-09-24 |
| P-557, `spec/data/above.4x` states what the naming rule already fixes                                                        | `spec/data/above.4x` - the file                                                                                                                                                                          | 2026-09-25 |
| P-558, a test is stated in the friendly form, and the foundation form is a rendering of it                                   | `spec/README.md` -> Rules for this directory, rule 3                                                                                                                                                     | 2026-09-25 |
| P-551, Four kinds of typed line, named for what each changes                                                                 | `spec/console.md` -> Commands, and `spec/interface.md` -> Logs                                                                                                                                           | 2026-09-25 |
