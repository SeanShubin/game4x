...
**asks** approval - **into** `docs/process.md` -> What I read, and what I do

**You answered collision 1 of `P-237` and the answer is better than the contradiction was.** It is a
naming issue, and the reason behind it is the organizing principle of the whole arrangement.
...
**Basis: it settles the word and gives the reason the document was missing.** *The specification
instance is the only one that writes a proposal* was true and read as a restriction. **It is not a
restriction, it is a definition** - the lanes may raise anything with each other, and only one kind
of thing reaches you.

**It also explains a rule already in the document.** *That keeps my queue to one author and one
length, which is what makes it reviewable* - and the sentence above says why one length matters at
all.

### P-239 - `pending.md` is built for you and you have not been reading it

**to** sean - **status** open - **raised** 2026-09-04 - **kind** contradiction - **asks** a decision
- **into** `docs/process.md` -> Specification Instance, and Outboxes and the index

**Where it is: `pending.md`, in the repository root**, linked from `README.md`. Regenerated by
`hooks/pre-commit` at every commit. Four sections: *What must be decided*, *Open and a commit says
otherwise*, *What is outstanding* - **19 items open to code and 1 to spec right now** - and
*Sections that have taken more than one proposal*.

**For you, the first section duplicates the proposal queue.** *What must be decided* is the open
...
stuck. Your own document says *it should not be possible to have work outstanding that needs nobody's
decision and is not being worked on... I must be able to see it*.

**This lane recommends 1, with the gap named rather than ignored.** Dropping it loses exactly that
one guarantee, so **the specification lane reports it instead** - one sentence when a lane is blocked
or idle, in the reply you were reading anyway. That keeps the guarantee and costs you no file.

**What it does not cost.** Nothing else changes: `pending.md` is generated either way, and the lanes
go on using it.

### P-240 - Who owns production support

**to** sean - **status** open - **raised** 2026-09-04 - **kind** Sean's own - **asks** a decision -
**into** `docs/process.md` -> Who writes what, and `CLAUDE.md` -> Perspectives

**The question is smaller than it looks, because most of it is already settled.** `CLAUDE.md` says
each lane owns the tools for its own work - `tools/spec/` is this lane's, `tools/<lens>/` is a lens's
- and that is not in dispute. **What has no owner is `hooks/`, `scripts/`, CI, and anything in
`tools/` shared between lanes.**

**And the current answer breaks a rule the same file relies on.** `CLAUDE.md` says production support
*belongs to no lane exclusively: whoever needs a check wired adds it*. It also says **one writer per
file is what makes running several instances at once safe**, because two instances editing one file
lose each other's edits silently. **Those cannot both hold.**

**Choice 1 - the coding lane.** It is code; the coding lane writes code. It is the lane whose work
stops when the gate breaks, so it has the strongest reason to keep it green. No new instance.
**Cost: this lane files a request and waits, where today it wires its own check.**

**Choice 2 - the quality lane.** Your own note gives it the charter: *makes sure we have automation
in our process instead of repetition*, *proper automated pipeline*, *proper support tooling*.
**Cost: a lens that ships something is no longer independent evidence**, and nothing reviews what it
ships.

**Choice 3 - a new lane for it.** Clean ownership and no compromise to either producer. **Cost: an
instance you have to run**, and `CLAUDE.md` asks of any new lane what it produces that neither
producer would.

**This lane recommends 1.** The deciding argument is the one-writer rule: **choice 1 is the only one
that costs nothing you are not already paying**, and quality keeps its independence by continuing to
ship nothing - *makes sure rules are enforced via reliable code* becomes a finding it files rather
than a thing it builds.
