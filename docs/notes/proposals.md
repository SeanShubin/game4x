...
**One**, and it is the mechanism you asked for.

### P-330 - A worked example beside every recipe, so one recipe can be confirmed on its own

**to** sean - **status** open - **raised** 2026-09-07 - **kind** Sean's own - **shape** text -
**asks** approval - **into** `releases/first-release.md` -> Capabilities, as `R-7`

**A capability rather than a rule, because what you are asking for is something you can look at and
say yes to.** The other six in that section have the same shape and this one is checked the same
way - by you, on the running thing.
...
**Why generated and never drawn.** A written example can show behaviour the code does not have, and
this repository has produced three of those in one week - a requirement no kind could satisfy,
quantities the model never computed, and a value the release never named. **All three sat in
artifacts whose purpose was hand-derivation**, which is exactly what this is.

**Why the boundaries and not one typical case.** Sixteen recipes; **five carry an expression rather
than a number**. `grow` is the sharp one: its rule bends at three points - food below the citizens,
food equal to them, food above twice them - and one example shows one of those while looking
complete. **The other eleven need one example each.**

**What this does not add.** The generic form already exists and is not being duplicated:
`reports/recipes.md` gives every recipe in placeholder terms already, expressions included. **The
example is what can be checked; the rule is what it is checked against**, and putting them beside
each other is what stops one drifting from the other.

**One thing worth knowing about what exists.** The behaviour is already confirmed per recipe - there
are twenty-nine tests in `game.rs` alone, named `landing_an_ark_founds_the_territory` and the like.
**They are correct and they are Rust**, so they sit outside the surface you vet. This capability does
not replace them; it renders what they already establish into something you can read.
