...
**Worth keeping for how the question had to be re-asked.** This lane asked what to call one
kind of command and Sean answered that he could not name one in isolation. **He was right
for a measurable reason**: `spec/console.md` already distinguished four kinds of typed line
and two of the four had no name, so the fifth had nothing to sit beside.

**And the answer cost no vocabulary, because he had already chosen it.** `game state` and
`local state` were settled hours earlier, and the command names are those two adjectives;
...
## The working behind `P-555`, answered on 2026-09-24

**Not an item** - `P-555` is a proposal and lives in
[`decide/proposals.md`](../../decide/proposals.md).

**Worth keeping for what `../boardgame` taught rather than for the decisions.** Its
`log-driven/README.md` makes a log a review surface - *vetting that the combat logs represent
my combat system* - and its `screen.txt` is an exhibit a machine can read, which is better
than the generated screenshot page this lane had proposed the same morning. **A screenshot
needs his eye and a screen dump needs nobody's.**

**And its habits are ones this repository already pays for**: the count in the section
heading, *26 strings attempted* for what the renderer tried rather than what appeared, and
...
**Sean answered by describing the loop rather than by choosing a letter**: *we start the
game with an ark, we deploy the ark to the surface, we develop the territory we deployed
to, we expand to an adjacent territory, we develop the adjacent territory, we launch an
ark from there.* **Six steps, in order, and the launch is last** - which is `W2`.

**It also says more than the win condition needs.** *Adjacent* is how expansion works
rather than a condition on winning, and *develop* is what a Yard costs rather than a
...
## The working behind `P-549`, answered `K1` on 2026-09-24

**Not an item** - `P-549` is a proposal and lives in
[`decide/proposals.md`](../../decide/proposals.md).

**Worth keeping because the answer was larger than the question.** The item asked which of
four statements was right. Sean answered that one and **added a rule nothing in `spec/`
states**: *an ark is never on the surface.* That sentence made `spec/data/limit.4x`'s ark
...
## The working behind `P-547`, answered `A1` on 2026-09-24

**Not an item** - `P-547` is a proposal and lives in
[`decide/proposals.md`](../../decide/proposals.md).

**What is worth keeping is that this item shrank twice and both times because a document
was read further.** It opened with three answers and four violations. Reading `The layers`
rather than only `Rules` excused two of the four and left two answers. Then the
contradiction sweep for promotion found that `A1` alone leaves rule 4's own sentence
saying *the adapter*, singular - **so the answer needed a second block the question never
offered.**

**The lesson is about where the rule was read from.** Rule 4 is a summary and the layer
table is the model; this lane cited the summary and reported four defects, two of which
