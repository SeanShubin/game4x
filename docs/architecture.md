...
| Layer             | Crates                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Supporting crates | `command-language`, `friendly-notation`, `game-console`, `game-front`, `game-model`, `graph-coloring`, `planet-model`, `planet-terrain`, `sphere-tessellation`, `thin-engine` |
| Rendering         | `planet-presentation`, `planet-raster`, `planet-render`                                                                                                                       |
| Engine adapter    | `game-globe`, `game-inspect`, `planet-bevy`, `planet-ecs`, `planet-flat`                                                                                                      |
...
**Eighteen crates, and every one is in exactly one layer.** The prototypes under `prototypes/`
...
| [`crates/game-inspect`](../crates/game-inspect/README.md)               | view       | `bevy`, `game-console`, `game-front`, `planet-bevy`                                                     | The remote control: put the camera at stated angles, choose a drawing, run commands, wait for the world to settle, write a PNG and a dump, quit. It drives the shipped binary, because a harness running a special path would be evidence about the harness                                                                                                                                                                                           |
...
| [`prototypes/kinds`](../prototypes/kinds/README.md)                     | binary     | **none**                                                                                                | The release's own tables as Rust data - the kinds, families, bins, traits and recipes - and a test holding every cell against the document. It does not play                                                                                                                                                                                                                                                                                          |
| [`prototypes/gap-view`](../prototypes/gap-view/README.md)               | binary     | `sphere-tessellation`                                                                                   | The question. Can a player set a destination anywhere on the world with one mouse gesture, without rotating anything? A flat map needs only the click, and the tax a flattening pays is a fixed 720 degrees of angular defect - so a layout chooses where to pay it, never how much                                                                                                                                                                   |
| [`prototypes/hex-torus-view`](../prototypes/hex-torus-view/README.md)   | binary     | `graph-coloring`                                                                                        | The question. At the sizes the game would use, does a flat isotropic hex torus read as a world, and how visible is the wrapping? One crate where `goldberg-view` takes four: a flat torus needs no sphere tessellation                                                                                                                                                                                                                                |
| [`crates/thin-engine`](../crates/thin-engine/README.md)                 | algorithm  | **none**                                                                                                | Runs a rule from rows of data and names no noun the game has. The engine the mainline's rules move to, and the only crate whose isolation from the game is checked rather than intended                                                                                                                                                                                                                                                               |
| [`crates/friendly-notation`](../crates/friendly-notation/README.md)     | algorithm  | `thin-engine`                                                                                           | The bridge between the form the engine runs and the form a person writes. **A crate rather than a module of the engine**, because `tests/isolation.rs` lets the engine name no noun the game has and this names a scout, a territory and a place. **A crate rather than a `#[path]` include**, because a dependency cannot point backwards: the review application reaches it, and **a record's meaning must not change because a rendering changed** |
| [`prototypes/goldberg-move`](../prototypes/goldberg-move/README.md)     | binary     | `bevy`                                                                                                  | Lays out a move of any length across a sphere by clicking, refusing a destination that several shortest routes reach. A gesture rather than a rule                                                                                                                                                                                                                                                                                                    |
