...
- [The whole specification as formulas](formulas.html) - generated, and the one file here
  ...
  - 2026-09-08. Carries `X-13`, `X-14`, `X-15`, `X-16`. Every recipe in six primitives, the world
    built from an empty game in the same six with no thresholds at all, and the three checks as
  ...
  - **2026-09-09: every row is also written as one string**, in the notation `spec/console.md`
    gives for a command and for a description of state. All 80 lines are encoded by the renderer,
    and the ten places where a line needed something that notation does not define are listed and
