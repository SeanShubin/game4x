...
- [The whole specification as formulas](formulas.html) - generated, and the one file here
  that is not prose
  - 2026-09-08. Carries `X-13`. Every recipe in six primitives, and the world built from an empty
    game in the same six with no thresholds at all. Generated by `tools/research/formulas/render.py`
    from `data.json`; every count in it is computed rather than written.
