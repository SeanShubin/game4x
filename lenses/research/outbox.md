...
### X-37 - Solium Infernum's wrap is not isotropic, and Sean found it legible anyway

**to** spec · **status** **acted** 2026-09-13 · `fd60a35` (the offset family) and `08cde7c` (the orientation toggle) — thirty worlds now, three families of ten, with `the_two_walks_sean_measured_are_the_offset_generators` asserting the derivation. Tests run here, not taken on report · **raised** 2026-09-13 · **source** Sean measuring a real game rather than reasoning about one: *12 spaces up returns to start, and 12 spaces right - alternating up-right, down-right - returns to start* · **found by** looking at something that shipped

**His two loops are the lattice, and they derive it exactly.** Twelve alternating rightward steps is
`6·(1,0) + 6·(1,−1) = (12, −6)`; twelve up is `(0, 12)`. So the wrapping lattice is generated by
**`(12, −6)` and `(0, 12)`** - which is `col mod 12, row mod 12` in offset coordinates, the wrap a
hex map stored as a two-dimensional array gives for free. 144 cells. **The conversion and his
measurement agree without being fitted to each other**, which is what makes this a derivation rather
than a guess.

**It is not isotropic, and that is the finding.** Straight-line circumference along the three hex
axes:

| lattice                | cells | circumference on the three axes |
| ---------------------- | ----- | ------------------------------- |
| **Solium Infernum**    | 144   | **24, 12, 24**                  |
| axis-aligned, `C = 12` | 144   | 12, 12, 12                      |
| folded, `k = 4`        | 48    | 12, 12, 12                      |

**One axis takes twice as long as another, and Sean did not notice until he went looking.** He
required equal circumference in all six directions on 2026-09-12 and a shipped game he found
legible does not have it. **So isotropy was never what made the pathing sensible.** What makes it
sensible is that the wrap happens in the coordinates a person thinks in - rows and columns - so
*twelve up* and *twelve right* are both twelve even though the straight axes are 24, 12, 24.

**`W` must be even.** `(W, −W/2)` needs `W/2` whole, which is the standard offset-coordinate
constraint: an odd column count breaks the half-row offset at the seam. It is likely why the map
Sean measured is twelve wide.

**Orientation is a separate axis and it is drawing only.** The prototype draws pointy-top hexes -
neighbours left and right - and Solium Infernum draws flat-top - neighbours up and down. **The
abstract grid is identical**; what changes is which walks are natural to a reader, which is why it
is not merely cosmetic. A toggle is cheap and the two should be comparable side by side.

**And the sentence this whole enquiry was looking for is his.** *I can always go from one pole to
the other without crossing the equator.*

**That is the thing that cannot be hidden.** On a sphere every closed loop separates, so the equator
divides north from south and a pole-to-pole journey must cross it. On a torus a circumnavigating
loop separates nothing, so you walk around. **It is the same fact as *a circumnavigation cuts
nothing*, in the one form a player would actually meet it** - no counting, no measurement, one walk,
and it is the objection that survives every rendering choice.

**A defect it exposed that had already shipped, and this lane looked straight past it.** The
axis-aligned family drew **six** echoes round a **parallelogram** domain from the day it landed. Six
close round a hexagon; a parallelogram has eight neighbours, so two opposite corners stood open at
all ten of its sizes - through this lane reading the page, hovering it, screenshotting it twice, and
a quality review. **Nobody named it.** The offset lattice is sheared far enough that the notches
became wedges, which is the only reason anyone looked. Found by drawing, like everything else in
this item.
