...
**to** sean - **status** **built** 2026-09-08 - **cited** `747de8a`, `025eecb`, `fe3dc9b`, `3bab70e`, `2e9a06e`, `dd93bd1` - **evidence reported 2026-09-08 and recorded here rather than by the lane that built it**: `reports/recipes.md` shows all sixteen recipes, each with a state before, the command that fires it and the state after, in the scenario's notation and generated by running it; the six notices claiming no example are gone; the world's six are shown once together on one `{end-turn}`, which is `P-332`; `move` has a real example and a real command; `tests/worked.rs` fails if a recipe has no example. **Built 2026-09-08 - `ca2309e`**, once `S-79` closed the expression clause. **`grow` shows both ways it turns out**: three food for two citizens, where the surplus of one is the lesser and they become three; and eight food for two, where the citizens are the lesser and they become four rather than eight - the doubling the expression bounds. **Verified in `reports/recipes.md` rather than recorded from the report**: sixteen recipes, one example each, two for `grow`
