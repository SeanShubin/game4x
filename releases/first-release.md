...
**to** code - **status** open - **cited** `747de8a`, `025eecb`, `fe3dc9b`, `3bab70e`, `2e9a06e`, `dd93bd1` - **evidence reported 2026-09-08 and recorded here rather than by the lane that built it**: `reports/recipes.md` shows all sixteen recipes, each with a state before, the command that fires it and the state after, in the scenario's notation and generated by running it; the six notices claiming no example are gone; the world's six are shown once together on one `{end-turn}`, which is `P-332`; `move` has a real example and a real command; `tests/worked.rs` fails if a recipe has no example. **Not yet built**: one clause is unmet - see `S-79`
