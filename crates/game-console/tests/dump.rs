...
fn every_kind_is_named_and_only_two_namings_depend_on_the_scenario() {
    ...
    assert_eq!(
        Kind::ALL.len(),
        18,
        "eighteen kinds: `store` from `P-260`, `deposit` from `P-322`, `adjacency` from `P-334`, `game` from `P-351`, `fertility` from the saturating rewrite and `readiness` from `P-399`; the model has {}",
        Kind::ALL.len()
    );
