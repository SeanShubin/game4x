...
/* A folded `##` section - `S-123`. The summary stands where the heading stood, so a reader
   sees the same outline whether a section is open or closed. `<details>` is plain HTML and
   needs no script, which is what keeps `R-9` true. */
details > summary { margin: 2rem 0 .25rem; font-size: 1.1rem; font-weight: 600 }
