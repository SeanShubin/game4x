...
pub fn tables(game: &Game) -> Vec<Table> {
    ...
    // **`readiness` was the row after this one until `P-411`**, which stopped it being a
...
/// The reports another crate writes, which this one only renders a page for.
///
/// **One since `R-7`.** `recipes.md` was here because `prototypes/kinds` generated it from the
/// release alone; a worked example is *generated by running it*, and that crate depends on
