//! Pads markdown tables so columns align in monospace editors.
//!
//! ```text
//! pad-tables [PATH...]
//! ```
//!
//! Each PATH is a directory to walk or a single `.md` file. With no arguments it walks the
//! current directory. Walking skips `.git`, `target`, `node_modules`, and any dot-prefixed
//! directory. Only files that actually change are written.
//!
//! Taking explicit paths is what lets a pre-commit hook pass exactly the staged files
//! instead of reformatting the whole tree.
//!
//! Ported from `seans-arcade/examples/pad_tables.rs`. The padding algorithm below is
//! unchanged, so both repositories format tables identically.
...
fn main() {
    ...
    for target in &targets {
        ...
        if path.is_dir() {
            visit_dir(path, &mut changed_files);
        } else if path.is_file() {
            if path.extension().and_then(|e| e.to_str()) == Some("md") && process_file(path) {
    ...
    if changed_files.is_empty() {
        ...
        println!();
        println!("Changed {} file(s).", changed_files.len());
    }
}
