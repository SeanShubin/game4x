...
hardest.</strong></p>
<div class="scroll"><table><thead><tr><th>What</th><th>Written as</th><th></th></tr></thead>
<tbody>{"".join(f'<tr><td class="target">{r[0]}</td><td class="target">{r[1]}</td><td class="note">{r[2]}</td></tr>' for r in DATA["representable"])}</tbody></table></div>
<div class="callout">
<h4>Force is the biggest change to the game and the smallest to the language</h4>
<p>It is the thing standing between occupying a planet and conquering one, and <strong>it needs no
new construct at all</strong> - <code>require</code> already compares two expressions, and
<code>sum trait of {{…}}</code> is already one of them. What is missing is <em>lines in
recipes</em>, which is the cheapest kind of missing there is.</p>
<p>That is a consequence of the change Sean made an hour before asking. <strong>Before
<code>require</code> compared expressions, force would have needed a new primitive; after it, force
needs a line.</strong></p>
</div>
<div class="callout">
<h4>Storage is where <code>clamp</code> finally earns its place</h4>
<p><code>clamp</code> has been a declared attach value with <strong>no call site</strong> since the
column was written - <em>do as much as the state allows</em>. Filling a store is exactly that: put
in what fits and no more. <strong>So storage needs no new construct either</strong>, and it resolves
a candidate that has been waiting for a reason to exist.</p>
<p>The other half - losing what was not stored - is <code>each</code> over what the <em>territory</em>
holds directly, which containment already distinguishes from what a <em>store</em> holds. That
distinction is the one <code>X-20</code> settled, and it is doing work here for the first time.</p>
</div>
<div class="callout">
<h4>The expedition splits in two, and only half of it is free</h4>
<p><strong>A fixed size is writable today.</strong> A <em>move-two</em> recipe guards on
<code>count {{pioneer in from}} &ge; 2</code> and moves two. Nothing new is needed and nothing is
elegant about it either: three pioneers would want a third recipe.</p>
<p><strong>Any size is not.</strong> <code>some</code> binds a name over <em>one</em> member of a
...
<h2>Claiming and holding, against nature</h2>
<p>Sean, 2026-09-09: <em>it may be that the force contest does not exist in the current loop. I also
had intended the ark to be able to project 2 force underneath it in order to clear the way to set up
a settlement that only projects 1 force, as you need <strong>greater force to claim, equal force to
hold</strong>.</em></p>
<div class="scroll"><table><thead><tr><th></th><th>Test</th><th>What meets it</th></tr></thead>
<tbody>{"".join(f'<tr><td class="target"><strong>{r[0]}</strong></td><td class="target">{r[1]}</td><td class="note">{r[2]}</td></tr>' for r in DATA["claim_hold_rule"])}</tbody></table></div>
<p><strong>Worked over every biome, the calibration is exact and has one exception:</strong></p>
<div class="scroll"><table><thead><tr><th>Biome</th><th>Force of nature</th><th>An ark can claim it</th>
<th>A settlement can hold it</th><th></th></tr></thead>
<tbody>{"".join(f'<tr><td class="target">{r[0]}</td><td class="amt">{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td><td class="note">{r[4]}</td></tr>' for r in DATA["claim_hold"])}</tbody></table></div>
<div class="callout">
<h4>Everything at nature 1 works exactly; jungle is the one that does not</h4>
<p>An ark projects 2, claiming needs <em>more</em> than nature, and every biome but one is nature 1
&mdash; so one ark clears any of them, and a settlement projecting 1 holds what it cleared, because
holding needs only <em>equal</em>. <strong>Jungle is nature 2</strong>, so claiming it needs more
than 2 and an ark carries exactly 2. <strong>An ark alone cannot claim a jungle</strong>, and a
settlement projecting 1 cannot hold one either.</p>
<p>That may be the intended difficulty rather than a gap - two units projecting together would carry
