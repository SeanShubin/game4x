...
match, literals assign*, offered as a guess and not a recommendation. **It cannot work**:
`build-extractor` already uses a literal to *match* - `{literal ... value:labor}` is how its remove
clause finds labor. **Which a value came from cannot decide what it is for.** So the split is its
own statement:
