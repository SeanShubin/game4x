...
<style>
 ...
 header {
   padding: 12px 16px; border-bottom: 1px solid #dde2e8; background: #fff; flex: none;
 }
 ...
 #pick { margin-top: 8px; display: flex; gap: 4px; flex-wrap: wrap; align-items: baseline; }
 ...
 #reach { color: #8f2626; font-weight: 600; }
</style>
<header>
 ...
 that look far apart here may be adjacent through a wrap. <span id="reach"></span></p>
 <p><strong>Twenty worlds are drawn, and the buttons below pick one</strong> &mdash; ten
...
function light(cell) {
  ...
  reach.textContent = `territory ${cell} is drawn in ${places} places — the nearest one decides the distance`;
}
...
<div style="padding: 8px 16px; flex: none; overflow: auto">
<table>
