...
fn main() {
    ...
    // **`recipes` moved to `game-console` with `R-7`.** A worked example is generated by
    // running the game, and this crate depends on nothing - which is the whole value of the
    // comparison it does. `kinds::recipes_table` still holds this crate's copy of the Recipes
    // table against the document; only the rendering left.
