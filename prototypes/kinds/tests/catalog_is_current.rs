...
// **`recipes.md` is held by `game-console` since `R-7`.** It carries a worked example under
// each rule now - a real command run against a real state - so it is generated by the crate
// that can run one, and `tests/dumps_are_current.rs` holds it current there. This crate still
// holds its own copy of the Recipes *table* against the document, which is `S-4` and is in
// `against_the_release.rs`.
